...
#######################################################################################################################
    ...
    async def cmd_skip(self, player, channel, author, message, permissions, voice_channel, param=''):
        ...
        if player.is_stopped:
            raise exceptions.CommandError(self.str.get("재생하는 음악이 없어서 스킵할수 없따우"), expire_in=20)
    ...
    async def cmd_quit(self, guild):
        ...
        await self.safe_send_message(channel, "`{0.name}`에서 나왔따우")
    ...
    async def cmd_stop(self, channel):
        ...
        """
        await self.safe_send_message(channel, "음악을 껐따우")
